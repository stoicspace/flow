...
export async function POST(request: Request) {
  ...
  try {
    ...
    const documentation = await documentWorkflow(provider, snapshot.normalised as NormalisedWorkFlow);

    // Best-effort persistence — safe no-op if the column doesn't exist yet.
    try {
      await storage.updateSnapshot(teamId, snapshot.id, { ai_documentation: documentation });
    } catch (persistErr) {
